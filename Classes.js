...
classes.furnace = class extends classes.machine {
  constructor(amount = 0) {
    ...
    this.inventory = {
      fuel: new Slot(
        ...
        { emptyClass: "fuelgui", onPostClick: this.doRecipe.bind(this) }
      ),
      input: new Slot(
        "",
        1,
      ...
      output: new Slot(
        ...
        { canPutItems: false, onPostClick: this.doRecipe.bind(this) }
      ),
    }
  }
  onCopy() {
  ...
  generateGui() {
    ...
    this.arrow.className = "furnacearrow guiArrowOff"
    this.handler.appendChild(this.arrow)
    for (const key in this.inventory) {
    ...
    if (this.gui) {
      ...
      if (this.recipe != undefined && this.recipetimer == -1) {
        if (this.checkvalidnes()) {
          this.arrow.className = "furnacearrow guiArrowOn"
  ...
  checkvalidnes() {
    if (this.recipe != undefined) {
      if (
        ...
        return false
      if (this.inventory.output.getItem().getEmpty() < this.recipe.amount)
        return false
...
classes.leathersword = class extends classes.tool {
  constructor(amount = 0) {
    ...
    this.type = "sword"
    this.description2 =
      "Ability: Gachi-Muchi".color("yellow") + br + "Summons Dungeon Master"
  }
}
...
classes.undeadsword = class extends classes.tool {
  constructor(amount = 0) {
    ...
    this.type = "sword"
    this.description2 =
      "Ability: Undead Enemy".color("yellow") +
      br +
      "Deals " +
      color("+100%", "lime") +
...
classes.zombiehat = class extends classes.item {
  ...
  Activate() {
    mobs.forEach((x) => {
      if (x.mobtype == "undead")
        this.additionalstats.defense = 20 + (this.additionalstats.defense >> 0)
    })
  }
  DeActivate() {
...
classes.enchantingpaste = class extends classes.item {
  constructor(amount = 0) {
    ...
    this.enchants = {}
    this.maxStackSize = 1
    this.description = "Stores Enchants,<br> Use Enchanting Book to Apply to tools"
  }
}
...
classes.enchantingbook = class extends classes.machine {
  constructor(amount = 0) {
    ...
    this.inventory = {
      input2: new Slot(
        (item) => item.name == "enchantingpaste",
        1,
        {
          position: "absolute",
          top: "8vh",
        ...
        { emptyClass: "enchantingPasteGui" }
      ),
      input: new Slot((item) => item.maxStackSize == 1, 1, {
        position: "absolute",
        top: "8vh",
        left: "20%",
      }),
  ...
  doRecipe() {
    if (this.gui) {
      if (this.inventory.input.isEmpty() || this.inventory.input2.isEmpty()) {
      ...
      this.applyButton.innerHTML =
        ...
        br +
        (getEnchantCost(
          this.inventory.input.getItem(),
          this.inventory.input2.getItem()
        ) +
        " Coins").color("yellow")
  ...
  doRecipeInternal() {
    if (this.gui) {
      ...
      if (
        ...
        for (const key in this.inventory.input2.getItem().enchants) {
          if (
            canEnchantApply(
              this.inventory.input.getItem(),
              key,
              this.inventory.input2.getItem().enchants[key]
            )
          ) {
            if (
              this.inventory.input2.getItem().enchants[key] ==
              this.inventory.input.getItem().enchants[key]
            ) {
              this.inventory.input.getItem().enchants[key]++
            } else
              this.inventory.input.getItem().enchants[key] =
                this.inventory.input2.getItem().enchants[key]
            delete this.inventory.input2.getItem().enchants[key]
          }
        }
        if (Object.keys(this.inventory.input2.getItem().enchants).length == 0)
          this.inventory.input2.putItem(new classes.empty())
        this.doRecipe()
      }
    }
  }
  generateGui() {
    this.handler = document.createElement("div")
    this.handler.className = "enchantingBookGui"
    ...
    arrow.className = "enchantingBookArrow"
    this.handler.appendChild(arrow)
    this.applyButton = document.createElement("button")
    this.applyButton.className = "enchantingbookButton"
    this.applyButton.innerText = "Apply"
    this.applyButton.onclick = this.doRecipeInternal.bind(this)
    this.handler.appendChild(this.applyButton)

    e.gui.appendChild(this.handler)
    this.gui = new Gui(
      "backpack",
      [$("#inventory")[0], $("#hotbar")[0], this.handler, $("#coins")[0]],
      toArray(this.inventory).concat(steve.inventory),
      {
        onopen: function () {
...
classes.glitchcompactor = class extends classes.machine {
  constructor(amount = 0) {
    ...
    this.inventory = {
      input1: new Slot("", 1, undefined, {
        onPostClick: this.doRecipe.bind(this),
      }),
      input2: new Slot("", 1, undefined, {
        onPostClick: this.doRecipe.bind(this),
      }),
      input3: new Slot("", 1, undefined, {
        onPostClick: this.doRecipe.bind(this),
      }),
      input4: new Slot("", 1, undefined, {
        onPostClick: this.doRecipe.bind(this),
      }),
      input5: new Slot("", 1, undefined, {
        onPostClick: this.doRecipe.bind(this),
      }),
      output: new Slot(
        "",
  ...
  generateGui() {
    ...
    for (const key in this.inventory) {
      ...
      } else this.handler.appendChild(this.inventory[key].getTag())
    }
    e.gui.appendChild(this.handler)
    this.gui = new Gui(
      "glitchcompacter",
...
classes.supercompactor = class extends classes.machine {
  ...
  buildinventory() {
    ...
    for (let i = 0; i < this.inventorySlots; i++) {
      this.inventory["input" + i] = new Slot(
        (item) => item.name.includes("glitched"),
        1,
        undefined,
        {
          onPostClick: this.doRecipe.bind(this),
  ...
  generateGui() {
    ...
    e.gui.appendChild(this.handler)
    this.gui = new Gui("supercompactor", [
      $("#inventory")[0],
      $("#hotbar")[0],
      this.handler,
    ])
...
classes.revenantset = class extends classes.armor {
  ...
  slayerArmorLevelUp() {
    while (slayerArmorMilestones[this.lvl + 1] <= this.kills) {
      ...
      this.additionalstats.zombiedefense =
        slayerArmorMilestonesDefense[this.lvl]
    }
  }
...
classes.projectile = class {
  ...
  startMovingToTarget() {
    ...
    let hipon = Math.sqrt(
      (this.x + this.size / 2 - this.target.x) ** 2 +
        (this.y + this.size / 2 - this.target.y) ** 2
    )
  ...
  move(vectorspeed) {
    ...
    let wasHit = false
    if (
      checkMapForBlock((this.y / 5) >> 0, ((this.x + this.size / 2) / 5) >> 0)
    ) {
      wasHit = true
      this.hit("wall")
    } else if (this.sender.name == "steve") {
      for (let i = 0; i < mobs.length; i++) {
        wasHit =
          distnaseBetweenTargets(this.getCenterCoords(), {
            x: mobs[i].x + mobs[i].width / 2,
            y: mobs[i].y + mobs[i].height / 2,
          }) < 4 && mobs[i].isAlive
        if (wasHit) {
          this.hit(i)
          i = 1000
      ...
      wasHit =
        distnaseBetweenTargets(this.getCenterCoords(), {
          x: steve.x + steve.width / 2,
          y: steve.y + steve.height / 2,
        }) < 2
      if (wasHit) {
        this.hit("steve")
      }
