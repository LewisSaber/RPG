...
const CollectionRewards = {
  ...
  logoak: [,,"Efficient Axe Recipe"],
  sugarcane: [,,"Paper Recipe"],
...
const CollectionRewardsCode = {
  cobblestone: [
   , ,
    "addShapedRecipe(['cobblestone', 'cobblestone', 'cobblestone', 'empty', 'stick', 'empty', 'empty', 'stick', 'empty'],'stonepickaxe')","addShapedRecipe(['cobblestone','cobblestone','cobblestone','cobblestone','empty','cobblestone','cobblestone','cobblestone','cobblestone'],'furnace',1,[].set(16, 9).put(1, 4))",
  ],
  rottenflesh: [ ,"addShapedRecipe(['empty', 'rottenflesh', 'empty', 'empty', 'rottenflesh', 'empty', 'ironingot', 'stick', 'ironingot'],'undeadsword')",,"addShapedRecipe(['zombiefang','rottenflesh','zombiefang','rottenflesh','ironhelmet','rottenflesh'],'zombiehat',1,[2,16,2,16,1,16,0,0,0])"],
  logoak: [,,"addShapedRecipe(['logoak', 'logoak', 'planksoak', 'logoak', 'stick', 'empty', 'empty', 'ironingot', 'empty'],'efficientaxe',1,[16,16,16,16,8,0,0,4,0])"],
  sugarcane: [,,' addShapelessRecipe(["sugarcane","sugarcane","sugarcane"],"paper",3)'],
...
function levelUpSkill(skill) {
  ...
  if (steve.skillxp[skill] >= skilllevelxp[steve.skilllevels[skill]]) {
    ...
    e["skilllevel" + skill].innerText = RomanNumerals.toRoman(steve.skilllevels[skill])
    if (loaded) {
      ...
      switch (lvl) {
        case 2:
         if(l == 0)
         {
          ...
          addShapedRecipe(["leather","leather","leather","leather","empty","leather"],"leatherhelmet",1,[3,3,3,3,0,3])

          addShapedRecipe(["leather","empty","leather","leather","leather","leather","leather","leather","leather",],"leatherchestplate",1,[3,0,3,3,3,3,3,3,3])

          addShapedRecipe(["leather","leather","leather","leather","empty","leather","leather","empty","leather"],"leatherleggins",1,[3,3,3,3,0,3,3,0,3])

          addShapedRecipe(["leather","empty","leather","leather","empty","leather"],"leatherboots",1,[3,0,3,3,0,3])
...
function levelUpCollection(item){
  if(steve.collectionitems[item] >= getCollectionAmounts(item)[steve.collectionlevels[item]+1])
  {
...
function giveCollectionReward(item,lvl){
  ...
  if(CollectionRewardsCode[item][lvl] != undefined && CollectionRewardsCode[item][lvl] != "")
eval(CollectionRewardsCode[item][lvl])
...
function loadCollections(){
  collections.forEach(x => {while (levelUpCollection(x)) {} })
}
